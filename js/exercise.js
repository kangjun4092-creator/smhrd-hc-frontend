...
// ---- 스쿼트 최저점 실루엣(이미지 기반): 손으로 그린 도형 대신, squat-bottom-ref.png(레퍼런스
// 사진에서 배경 제거한 것)를 흰색 실루엣으로 씌운다. 밝은 배경(스튜디오 흰 배경)은 투명 처리,
// 어두운 부분(사람)은 흰색으로 통일하는 방식이라 배경이 단색일 때만 깨끗하게 나온다.
// 방금 끝난 렙의 등급(PERFECT/GREAT/GOOD/MISS)에 맞춰 색을 미리 4벌 만들어 캐싱해둔다.
let squatBottomSilhouette = null; // 로드 실패/전이면 null·false → 그때는 기존 도형 방식으로 폴백
let exLastRepGrade = null; // 방금 끝난 렙의 등급. 아직 없으면 null → 실루엣은 흰색(디폴트)

function resolveCssColor(value) {
  // gradeColor()는 'var(--accent)' 같은 CSS 변수 문자열을 주는데 canvas는 var()를 못 읽어서 직접 풀어준다.
  const m = /^var\((--[\w-]+)\)$/.exec(value.trim());
  if (!m) return value;
  return getComputedStyle(document.documentElement).getPropertyValue(m[1]).trim() || '#ffffff';
}
function buildWhiteSilhouetteCanvas(img) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    // 흰 배경뿐 아니라, 발밑에 깔린 무채색 운동매트(회색, R≈G≈B)도 배경으로 취급해 지운다 —
    // 안 그러면 매트가 밝기 기준을 못 넘어서 사람 몸통과 이어진 회색 덩어리로 같이 칠해진다.
    // 채도가 낮은(R,G,B가 서로 비슷한) 중간 밝기 영역만 걸러내므로, 채도가 있는 피부색·옷은
    // 안전하게 살아남는다.
    const colorSpread = Math.max(r, g, b) - Math.min(r, g, b);
    const isBrightBg = brightness > 235;
    const isFlatGrayMat = brightness > 110 && brightness < 225 && colorSpread < 14;
    if (isBrightBg || isFlatGrayMat) { d[i + 3] = 0; } // 배경/매트 → 투명
    else { d[i] = 255; d[i + 1] = 255; d[i + 2] = 255; } // 사람 → 흰색(경계 알파는 유지)
  }
...
function tintSilhouette(baseCanvas, colorHex) {
  const c = document.createElement('canvas');
  c.width = baseCanvas.width; c.height = baseCanvas.height;
  const ctx = c.getContext('2d');
  ctx.drawImage(baseCanvas, 0, 0);
  ctx.globalCompositeOperation = 'source-in'; // 실루엣 알파 모양은 유지한 채 색만 덧입힌다
  ctx.fillStyle = colorHex;
  ctx.fillRect(0, 0, c.width, c.height);
  return c;
}
async function ensureSquatBottomSilhouette() {
  if (squatBottomSilhouette) return squatBottomSilhouette;
  try {
    const img = new Image();
    img.src = 'squat-bottom-ref.png';
    await img.decode();
    const white = buildWhiteSilhouetteCanvas(img);
    const bbox = opaqueBBox(white);
    squatBottomSilhouette = {
      bbox, white,
      PERFECT: tintSilhouette(white, resolveCssColor(gradeColor('PERFECT'))),
      GREAT: tintSilhouette(white, resolveCssColor(gradeColor('GREAT'))),
      GOOD: tintSilhouette(white, resolveCssColor(gradeColor('GOOD'))),
      MISS: tintSilhouette(white, resolveCssColor(gradeColor('MISS'))),
    };
  } catch (err) {
    console.warn('squat-bottom-ref.png를 못 찾았습니다 — 기존 도형 실루엣으로 표시됩니다', err);
    squatBottomSilhouette = false; // 다시 시도하지 않도록 "없음"을 기억
  }
...
function exDrawSquatBottomGhostImage(ctx, w, h) {
  ...
  const key = exLastRepGrade && sil[exLastRepGrade] ? exLastRepGrade : 'white';
  ctx.save();
  ctx.globalAlpha = 0.55;
  ctx.drawImage(sil[key], sil.bbox.x, sil.bbox.y, sil.bbox.w, sil.bbox.h, dx, dy, drawW, drawH);
  ctx.restore();
}

function exDrawSquatBottomGhostShape(ctx, w, h) {
  const profile = state.user.calibration;
  if (!profile || !profile.landmarks) return;
  const pts = profile.landmarks;
...
function exDrawSquatBottomGhost(ctx, w, h) {
  if (squatBottomSilhouette) { exDrawSquatBottomGhostImage(ctx, w, h); return; }
  exDrawSquatBottomGhostShape(ctx, w, h); // 이미지가 없거나 아직 로드 전이면 기존 도형으로 폴백
}
...
function exFlashGrade(grade) {
  ...
  // 등급 텍스트가 사라지는 타이밍에 맞춰 실루엣 색도 같이 기본(흰색)으로 되돌린다.
  exFlashGrade._tid = setTimeout(() => { el.classList.remove('show'); exLastRepGrade = null; }, 900);
}
      ...
      if (state.exercise.camPhase === 'recording') {
        ...
        if (result.ok) {
          ...
          if (!exFinalCountdownActive && heldMs >= CAM_ALIGN_HOLD_MS) {
            ...
            // speakFeedback()은 새로 말할 때 이전 발화를 즉시 취소한다 — 바로 이어서 카운트다운의
            // "삼"을 부르면 "시작합니다"가 다 끝나기도 전에 잘려서 겹쳐 들린다. 이 문장이 끝날
            // 시간을 벌어준 뒤에 카운트다운을 시작한다.
            setTimeout(startFinalCountdown, 1200);
          }
        } else {
          exAlignedSince = null;
          showReadyRing(false);
...
function beginRecording() {
  ...
  exRecordedChunks = [];
  try {
    // iOS Safari는 webm을 못 만들고 mp4만 지원한다 — 브라우저가 실제로 지원하는 포맷을
    // 순서대로 찾아서 쓰고, 그 실제 포맷을 exRecordedMimeType에 기억해뒀다가 Blob 만들 때 그대로 쓴다.
    const candidates = ['video/webm;codecs=vp9', 'video/webm;codecs=vp8', 'video/webm', 'video/mp4'];
    const supported = candidates.find(t => MediaRecorder.isTypeSupported && MediaRecorder.isTypeSupported(t));
    exMediaRecorder = supported ? new MediaRecorder(state.exercise.camStream, { mimeType: supported })
                                 : new MediaRecorder(state.exercise.camStream);
    exRecordedMimeType = exMediaRecorder.mimeType || supported || 'video/webm';
...
function stopRecording() {
  ...
  if (isSquat && exMediaRecorder && exMediaRecorder.state !== 'inactive') {
    exMediaRecorder.onstop = () => {
      ...
      state.exercise._pendingVideoUrl = exRecordedChunks.length ? URL.createObjectURL(new Blob(exRecordedChunks, { type: exRecordedMimeType })) : null;
      finish();
    };
...
async function saveExerciseResult() {
  ...
  try {
    ...
    await loadMyProfile(); // 서버가 계산한 실제 포인트·레벨·오늘 세트 사용량으로 다시 채움
