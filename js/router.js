...
async function setMenu(id){
  // 홈크루 메뉴를 벗어날 때는(어느 서브탭에 있었든) 채팅·크루원 실시간 소켓 연결을 끊는다 —
  // 반대로 메뉴에 들어와 있는 동안은 서브탭을 옮겨다녀도 계속 연결을 유지해서, 크루채팅
  // 탭이 아니어도 강퇴 같은 멤버십 변경을 실시간으로 받을 수 있게 한다(connectCrewChat 참고).
  if(state.menu==='crew' && id!=='crew') disconnectCrewChat();
  state.menu=id;
  saveSessionMenu(id); // 새로고침하면 마지막으로 보던 메뉴로 돌아오도록 기억해둔다
  render();
  if(id==='crew'){
    await loadMyCrew();
...
function goHome() { if(state.menu==='crew') disconnectCrewChat(); state.menu = 'main'; saveSessionMenu('main'); render(); }
